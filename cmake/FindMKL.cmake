...
# Find MKL headers
#
find_path(MKL_INCLUDE_DIRS mkl.h HINTS ${MKL_ROOT}/include
...
#
find_package_handle_standard_args(MKL REQUIRED_VARS MKL_INCLUDE_DIRS
                                                    Threads_FOUND)
      ...
      foreach(_threading "SEQ" "OMP" "TBB")
        ...
        foreach(_mpi_impl "MPICH" "OMPI")
          ...
          if(_mkl_blacs_lib
              ...
              AND NOT TARGET MKL::blacs_${_tgt_config})
            set(_blacs_libs
...
if(MKL_FOUND AND NOT TARGET MKL::blas)
  ...
  if(BLAS_THREADING MATCHES "thread" OR BLAS_THREADING MATCHES "gnu-thread")
    set(BLAS_mkl_thread__ "omp")
  endif()

  if(BLAS_THREADING MATCHES "sequential")
    set(BLAS_mkl_thread__ "seq")
  endif()

  if(BLAS_THREADING MATCHES "intel-thread")
    set(BLAS_mkl_thread__ "intel")
  endif()

  if(BLAS_THREADING MATCHES "tbb")
  ...
  get_target_property(
    ...
    MKL::${BLAS_mkl_INTFACE}_${BLAS_mkl_ILP_MODE}_${BLAS_mkl_thread__}_dyn
  ...
  if(NOT TARGET MKL::blas)
    add_library(MKL::blas INTERFACE IMPORTED)
    add_library(MKL::MKL INTERFACE IMPORTED)
    # create a empty lapack
    add_library(MKL::lapack INTERFACE IMPORTED)
  endif()
  set_target_properties(
    MKL::blas PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${MKL_INCLUDE_DIRS}"
                         INTERFACE_LINK_LIBRARIES "${MKL_BLAS_LIBRARIES}")
  set_target_properties(
    MKL::MKL PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${MKL_INCLUDE_DIRS}"
                        INTERFACE_LINK_LIBRARIES "${MKL_BLAS_LIBRARIES}")
  ...
  get_target_property(
    __mkl_scalapack_inc
    MKL::scalapack_${__mkl_mpi_ver_}_${BLAS_mkl_INTFACE}_${BLAS_mkl_ILP_MODE}_${BLAS_mkl_thread__}_dyn
    INTERFACE_INCLUDE_DIRECTORIES)
  get_target_property(
    __mkl_scalapack_lib
    MKL::scalapack_${__mkl_mpi_ver_}_${BLAS_mkl_INTFACE}_${BLAS_mkl_ILP_MODE}_${BLAS_mkl_thread__}_dyn
  ...
  get_target_property(
    __mkl_blacs_inc
    MKL::blacs_${__mkl_mpi_ver_}_${BLAS_mkl_INTFACE}_${BLAS_mkl_ILP_MODE}_${BLAS_mkl_thread__}_dyn
    INTERFACE_INCLUDE_DIRECTORIES)
  get_target_property(
    __mkl_blacs_lib
    MKL::blacs_${__mkl_mpi_ver_}_${BLAS_mkl_INTFACE}_${BLAS_mkl_ILP_MODE}_${BLAS_mkl_thread__}_dyn
    INTERFACE_LINK_LIBRARIES)
  if(NOT TARGET MKL::scalapack_link)
    add_library(MKL::scalapack_link INTERFACE IMPORTED)
    set_target_properties(
      MKL::scalapack_link
  ...
  set(BLAS_VENDOR "MKL")
  set(MKL_SCALAPACK_VENDOR TRUE)
  mark_as_advanced(BLAS_VENDOR)
  mark_as_advanced(NKL_FOUND)
  mark_as_advanced(MKL_SCALAPACK_VENDOR)
endif()
