...
RUN apt-get install -y --no-install-recommends gcc g++ gfortran git make unzip file \
  ...
  apt-transport-https ca-certificates gnupg software-properties-common perl tar bzip2 python3 \
  python3-pip \
  python3-wheel \
  python3-setuptools \
  python3-dev \
...
ENV SPEC="cp2k@master%gcc build_type=Release +sirius +spla +libint2 smm=libxsmm +openmp +cosma +elpa +spglib +libvori +quip +pexsi +libxc +cuda ^mpich@${MPICH_VERSION} ^intel-oneapi-mkl+cluster ^spla+cuda ^spfft+cuda ^elpa+openmp ^cosma+cuda ^dbcsr+cuda"
