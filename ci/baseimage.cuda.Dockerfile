...
RUN spack install --only=dependencies --fail-fast \
  "cp2k@develop %gcc build_type=RelWithDebInfo +cuda +libxc +libint +mpi +spglib +smm=libxsmm ^mpich ^intel-oneapi-mkl+cluster"

RUN spack install --only=dependencies --fail-fast \
  "cp2k@develop %gcc build_type=RelWithDebInfo +cuda +libxc +libint +mpi +spglib +smm=libxsmm ^openmpi ^intel-oneapi-mkl+cluster"

RUN spack install --only=dependencies --fail-fast \
  "cp2k@develop %gcc build_type=RelWithDebInfo +cuda +libxc +libint +mpi +spglib +smm=libxsmm ^mpich ^openblas+fortran"

RUN spack install --only=dependencies --fail-fast \
  "cp2k@develop %gcc build_type=RelWithDebInfo +cuda +libxc +libint +mpi +spglib +smm=libxsmm ^openmpi ^openblas+fortran"
