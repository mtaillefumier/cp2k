...
RUN apt-get install -y gcc g++ gfortran clang git make unzip \
  ...
  patchelf meson perl tar bzip2 python3 \
  python3-pip \
  python3-wheel \
  python3-setuptools \
  python3-dev \
...
ENV SPEC="sirius@develop %gcc build_type=Release +enable_regtests +sirius +spla +libint2 smm=libxsmm +openmp +cosma +elpa +spglib +libvori +quip +pexsi +libxc ^openblas ^mpich"
