...
static inline void initialize_tensor(tensor *a, const int dim,
...
static inline void initialize_tensor_2(tensor *a, int n1, int n2) {
	if (a == NULL)
		return;
...
static inline void initialize_tensor_3(tensor *a, int n1, int n2,
																			 int n3) {
	if (a == NULL)
		return;
...
static inline void initialize_tensor_4(tensor *a, int n1, int n2,
																			 int n3, int n4) {
	if (a == NULL)
		return;
...
static inline size_t tensor_return_memory_size(const tensor *const a) {
	if (a == NULL)
		abort();
...
static inline void tensor_assign_memory(tensor *a, void *data) {
	if (a == NULL)
		abort();
	a->data = (double *)data;
}

static inline int tensor_get_leading_dimension(tensor *a) {
	if (a == NULL)
		abort();
	return a->ld_;
}

static inline void tensor_set_leading_dimension(tensor *a,
																								const int ld) {
	if (a == NULL)
		abort();
	a->ld_ = ld;
...
static inline void recompute_tensor_offsets(tensor *a) {
	if (a == NULL)
		abort();
