...
#include "non_orthorombic_corrections.hpp"
#include "tensor_local.hpp"
#include "utils.hpp"

void extract_cube_within_spherical_cutoff_ortho(
		collocation_integration *const handler, const double disr_radius,
				...
				if ((jremain >= 0.0) && ywindow.is_point_in_interval(j)) {
					...
					for (int x = xmin - lb_cube[2];
						...
						if (upper_corner[2] - lower_corner[2]) {
							...
							double *__restrict__ dst = &idx3(handler->cube, position1[0],
...
void extract_cube(collocation_integration *handler, const int cmax,
...
void grid_integrate(collocation_integration *const handler,
	...
	if (lp != 0) {
			tensor_reduction_for_collocate_integrate((double *)handler->scratch,
		...
		cblas_dgemv(CblasRowMajor, CblasNoTrans, handler->cube.size[0],
								...
								&idx3(handler->pol, 1, 0, 0), 1, 0.0, (double *)handler->scratch, 1);

		/* finally along k, it is a scalar product.... */
		handler->coef.data[0] = cblas_ddot(handler->cube.size[0], (double *)handler->scratch,
...
#pragma omp parallel default(shared)
	{
		...
		collocation_integration *handler = ctx->handler[thread_id];
...
extern "C" void grid_cpu_integrate_task_list(
	...
	for (int level = 0; level < ctx->nlevels; level++) {
		set_grid_parameters(&ctx->grid[level], orthorhombic, &npts_global[3 * level],
												&npts_local[3 * level], &shift_local[3 * level],
												&border_width[3 * level], &dh[9 * level], &dh_inv[9 * level],
												grid[level]);
	...
	for (int level = 0; level < ctx->nlevels; level++) {
		integrate_one_grid_level_dgemm(ctx, level, compute_tau, calculate_forces,
																	 calculate_virial, &shift_local[3 * level],
																	 &border_width[3 * level], pab_blocks, hab_blocks,
																	 &forces_, &virial_);
		ctx->grid[level].data = NULL;
	}
	if (calculate_forces) {
		if (calculate_virial) {
			virial[0] = idx2(virial_, 0, 0);
			virial[1] = idx2(virial_, 0, 1);
			virial[2] = idx2(virial_, 0, 2);
			virial[3] = idx2(virial_, 1, 0);
			virial[4] = idx2(virial_, 1, 1);
			virial[5] = idx2(virial_, 1, 2);
			virial[6] = idx2(virial_, 2, 0);
			virial[7] = idx2(virial_, 2, 1);
			virial[8] = idx2(virial_, 2, 2);
		}

		memcpy(forces, forces_.data, sizeof(double) * forces_.alloc_size_);
		free(forces_.data);
		free(virial_.data);
	}
