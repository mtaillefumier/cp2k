...
void exp_ij(const double alpha, const int offset_i, const int imin,
	...
	for (int i = 0; i < (imax - imin); i++) {
		double *__restrict__ dst = &idx2(exp_ij_[0], i + offset_i, offset_j);
			...
			for (int y = 0; y < (upper_corner[d1] - lower_corner[d1]); y++) {
				...
				for (int x = 0; x < (upper_corner[d2] - lower_corner[d2]); x++) {
					...
					for (int y2 = 0; y2 < block_size[d1]; y2++) {
						double *__restrict__ dst = &idx2(exp_blocked, y2, 0);
						const double scal = x1[y_1 + y2] * c_exp_const;
						const double *__restrict__ src = &x2[x_1];
#pragma GCC ivdep
	...
	if (plane[0] && plane[1]) {
		for (int z = 0; z < cube->size[0]; z++) {
			for (int y = 0; y < cube->size[1]; y++) {
				...
				double *__restrict__ dst = &idx3(cube[0], z, y, 0);
...
void apply_non_orthorombic_corrections_xy_blocked(
		const tensor *const Exp,  tensor *const m) {
	for (int gamma = 0; gamma < m->size[0]; gamma++) {
		for (int y1 = 0; y1 < m->size[1]; y1++) {
			double *__restrict__ dst = &idx3(m[0], gamma, y1, 0);
			const double *__restrict__ src = &idx2(Exp[0], y1, 0);
#pragma GCC ivdep
...
void apply_non_orthorombic_corrections_xz_blocked(
	...
	for (int z1 = 0; z1 < m->size[0]; z1++) {
		const double *__restrict__ src = &idx2(Exp[0], z1, 0);
		for (int y1 = 0; y1 < m->size[1]; y1++) {
			double *__restrict__ dst = &idx3(m[0], z1, y1, 0);
#pragma GCC ivdep
			for (int x1 = 0; x1 < m->size[2]; x1++) {
				dst[x1] *= src[x1];
			}
...
void apply_non_orthorombic_corrections_xz_yz_blocked(
		const tensor *const Exp_xz, const tensor *const Exp_yz,
		tensor *const m) {
	for (int z1 = 0; z1 < m->size[0]; z1++) {
		const double *__restrict__ src_xz = &idx2(Exp_xz[0], z1, 0);
		for (int y1 = 0; y1 < m->size[1]; y1++) {
