...
#include "collocation_integration.hpp"
#include "cpu_private_header.hpp"
#include "grid_collocate_dgemm.hpp"
...
void update_atoms_position(const int natoms,
	...
	if (data->atom_positions == NULL) {
			data->atom_positions = (double *)malloc(3 * natoms * sizeof(double));
	} else {
		if (natoms > data->natoms) {
				data->atom_positions =(double *)
	...
	if (data->atom_positions) {
		for (int i = 0; i < natoms; i++) {
			data->atom_positions[3 * i] = atoms_positions[3 * i];
			data->atom_positions[3 * i + 1] = atoms_positions[3 * i + 1];
			data->atom_positions[3 * i + 2] = atoms_positions[3 * i + 2];
		}
	}
}
...
void update_atoms_kinds(const int natoms, const int *atoms_kinds,
	...
	if (data->atom_kinds == NULL) {
			data->atom_kinds = (int *)malloc(natoms * sizeof(int));
	} else {
		if ((natoms > data->natoms) && (data->natoms > 0)) {
				data->atom_kinds = (int *)realloc(data->atom_kinds, natoms * sizeof(int));
		}
	}
...
void update_block_offsets(const int nblocks, const int *const block_offsets,
	...
	if (data->block_offsets == NULL) {
			data->block_offsets = (int *)malloc(nblocks * sizeof(int));
	} else {
		if ((nblocks > data->nblocks_total) && (data->nblocks_total > 0)) {
				data->block_offsets = (int *)realloc(data->block_offsets, sizeof(int) * nblocks);
		}
	}
...
void update_basis_set(const int nkinds, const grid_basis_set **const basis_sets,
	...
	if (nkinds > data->nkinds_total) {
		if (data->basis_sets == NULL) {
				data->basis_sets = (grid_basis_set **)malloc(nkinds * sizeof(grid_basis_set *));
		} else {
				data->basis_sets =(grid_basis_set **)
...
void update_task_lists(const int nlevels, const int ntasks,
											 ...
											 const double *rab_list, grid_context *ctx) {
	...
	if (ctx->ntasks == 0) {
		...
		ctx->tasks_per_level = (int *)malloc(size);
		ctx->tasks = (_task **)malloc(nlevels * sizeof(_task *));
		/* memset(ctx->tasks, 0, nlevels * sizeof(_task *)); */
		if (ntasks)
				ctx->tasks[0] =  (_task *)malloc(ntasks * sizeof(_task));
		else
			ctx->tasks[0] = nullptr;
	} else {
		if (ctx->nlevels_total < nlevels) {
			/* save the address of the full task list. NULL when completly empty */
				ctx->tasks = (_task **)realloc(ctx->tasks, nlevels * sizeof(_task *));
		}
		if (ctx->ntasks_total < ntasks) {
				ctx->tasks[0] = (_task *)realloc(ctx->tasks[0], ntasks * sizeof(_task));
		}
	}
	...
	for (int i = 0; i < ntasks; i++) {
		...
		task->rab[0] = rab_list[3 * i];
		task->rab[1] = rab_list[3 * i + 1];
		task->rab[2] = rab_list[3 * i + 2];
...
void update_grid(const int nlevels, grid_context *ctx) {
	...
	if (ctx->grid == NULL) {
			ctx->grid = (tensor *)malloc(sizeof(tensor) * nlevels);
	} else {
		if (ctx->nlevels_total < nlevels) {
				ctx->grid = (tensor *)realloc(ctx->grid, sizeof(tensor) * nlevels);
		}
	}
...
void *create_grid_context_cpu(
		...
		const double *atom_positions, const int *const atom_kinds,
		...
		const double *const radius_list, const double *rab_list) {

		grid_context *ctx = (grid_context *)malloc(sizeof(grid_context));
		...
		ctx->handler = (collocation_integration **)
				malloc(sizeof(collocation_integration *) * max_threads);
...
void initialize_grid_context_on_gpu(void *ptr, const int number_of_devices,
	...
	if (ctx->device_id == NULL)
			ctx->device_id = (int *)malloc(sizeof(int) * number_of_devices);
	else
			ctx->device_id = (int *)realloc(ctx->device_id, sizeof(int) * number_of_devices);
...
void set_grid_parameters(
	...
	grid->dh_inv[0][0] = dh_inv[0];
	grid->dh_inv[0][1] = dh_inv[1];
	grid->dh_inv[0][2] = dh_inv[2];
	grid->dh_inv[1][0] = dh_inv[3];
	grid->dh_inv[1][1] = dh_inv[4];
	grid->dh_inv[1][2] = dh_inv[5];
	grid->dh_inv[2][0] = dh_inv[6];
	grid->dh_inv[2][1] = dh_inv[7];
	grid->dh_inv[2][2] = dh_inv[8];
...
extern "C" void grid_cpu_create_task_list(
	...
	if (*task_list == NULL) {
			*task_list = (grid_context *)create_grid_context_cpu(
