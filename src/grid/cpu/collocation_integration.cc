...
collocation_integration *collocate_create_handle() {
	collocation_integration *handle = NULL;
	handle = (collocation_integration *)malloc(
			sizeof(collocation_integration));

	if (handle == NULL) {
		abort();
	}
	memset(handle, 0, sizeof(collocation_integration));
	...
	handle->scratch = (double *)malloc(sizeof(double) * 32768);
...
void collocate_destroy_handle(void *gaussian_handle) {
	collocation_integration *handle =
			(collocation_integration *)gaussian_handle;
	if (handle->Exp.data)
