...
void tensor_reduction_for_collocate_integrate(
		...
		const tensor *Exp, const tensor *co,
		const tensor *p_alpha_beta_reduced_, tensor *cube);
...
void apply_sphere_cutoff_ortho(collocation_integration *const handler,
...
void collocate_l0(double *scratch, const double alpha, const bool orthogonal_xy,
	...
	if (exp_xy && !orthogonal_xy) {
		for (int y = 0; y < cube->size[1]; y++) {
			const double *__restrict__ src = &idx2(exp_xy[0], y, 0);
			double *__restrict__ dst = &scratch[y * cube->ld_];
#pragma GCC ivdep
...
void tensor_reduction_for_collocate_integrate(
	...
	if (co->size[0] > 1) {
		...
		double *__restrict__ const pz =
				&idx3(p_alpha_beta_reduced_[0], 0, 0, 0); /* k indice */
		double *__restrict__ const py =
				&idx3(p_alpha_beta_reduced_[0], 1, 0, 0); /* j indice */
		double *__restrict__ const px =
				&idx3(p_alpha_beta_reduced_[0], 2, 0, 0); /* i indice */
...
void apply_mapping_cubic(collocation_integration *handler,
...
void grid_collocate(collocation_integration *const handler,
	...
	tensor_reduction_for_collocate_integrate(
			static_cast<double *>(handler->scratch), // pointer to scratch memory
...
void compute_coefficients(grid_context *const ctx,
													collocation_integration *handler,
 ...
 ******************************************************************************/
extern "C" void grid_cpu_collocate_task_list(
	...
	for (int level = 0; level < ctx->nlevels; level++) {
		collocate_one_grid_level_dgemm(ctx, &border_width[3 * level], &shift_local[3 * level],
