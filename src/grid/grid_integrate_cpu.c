...
extern void collocate_l0(double *scratch,
                         const double alpha,
                         const double beta,
...
extern void collocate_l0_blocked_xy(double *scratch,
                                    const double alpha,
                                    const int sizex,
                                    const int sizey,
                                    const int x0,
...
extern void collocate_l0_blocked_xy_add(double *scratch,
                                        const double alpha,
                                        const int sizex,
                                        const int sizey,
                                        const int x0,
...
double integrate_l0_z(double *scratch,
                      const int z0,
...
*/
void tensor_reduction_for_integrate_blocked(double *__restrict scratch,
                                            const double alpha,
    ...
    for (int y = 0; y < (upper_block[1] - lower_block[1]); y++) {
        ...
        for (int z = 0; z < (upper_block[0] - lower_block[0]); z++) {
            ...
            */
            ...
            m2[z].b = &idx3(p_alpha_beta_reduced_[0], 0, z * gr->blockDim[0], 0); // Z_{gamma, k}  = p_alpha_beta_reduced(0, k, gamma)
                    ...
                    const tensor *grid)
{
    ...
    if (use_ortho) {
        grid_fill_pol((lp != 0), handler->dh[0][0], roffset[2], 0, lb_cube[2], ub_cube[2], lp, cmax, zetp, &idx3(handler->pol, perm[2], 0, 0)); /* i indice */
        grid_fill_pol((lp != 0), handler->dh[1][1], roffset[1], 0, lb_cube[1], ub_cube[1], lp, cmax, zetp, &idx3(handler->pol, perm[1], 0, 0)); /* j indice */
        grid_fill_pol((lp != 0), handler->dh[2][2], roffset[0], 0, lb_cube[0], ub_cube[0], lp, cmax, zetp, &idx3(handler->pol, perm[0], 0, 0)); /* k indice */
    } else {
        double dx[3];
        ...
        grid_fill_pol((lp != 0), 1.0, roffset[2], 0, lb_cube[2], ub_cube[2], lp, cmax, zetp * dx[2], &idx3(handler->pol, perm[2], 0, 0)); /* i indice */
        grid_fill_pol((lp != 0), 1.0, roffset[1], 0, lb_cube[1], ub_cube[1], lp, cmax, zetp * dx[1], &idx3(handler->pol, perm[1], 0, 0)); /* j indice */
        grid_fill_pol((lp != 0), 1.0, roffset[0], 0, lb_cube[0], ub_cube[0], lp, cmax, zetp * dx[0], &idx3(handler->pol, perm[0], 0, 0)); /* k indice */
    ...
    if (lp != 0) {
        ...
        collocate_l0(handler->scratch,
                     ...
                     &handler->pol,
                     &cube_tmp);
        handler->coef.data[0] = cblas_ddot(cube_tmp.alloc_size_,
                                           cube_tmp.data, 1,
                                           handler->cube.data, 1);
    }
...
void grid_integrate_blocked(collocation_integration *const handler,
                            const bool use_ortho,
                            const int lp,
                            const double zetp,
                            const double rp[3],
                            ...
                            const tensor *grid)
{
    ...
    initialize_tensor_3(&handler->cube,
                        (upper_block_corner[0] - lower_block_corner[0]) * handler->blockDim[0],
    ...
    if (use_ortho) {
        ...
        grid_fill_pol((lp != 0), handler->dh[2][2], roffset[0], pol_offset[0], lb_cube[0], ub_cube[0], lp, cmax, zetp, &idx3(handler->pol, 0, 0, 0)); /* k indice */
    } else {
        double dx[3];
        ...
        grid_fill_pol((lp != 0), 1.0, roffset[2], pol_offset[2], lb_cube[2], ub_cube[2], lp, cmax, zetp * dx[2], &idx3(handler->pol, 2, 0, 0)); /* i indice */
        grid_fill_pol((lp != 0), 1.0, roffset[1], pol_offset[1], lb_cube[1], ub_cube[1], lp, cmax, zetp * dx[1], &idx3(handler->pol, 1, 0, 0)); /* j indice */
        grid_fill_pol((lp != 0), 1.0, roffset[0], pol_offset[0], lb_cube[0], ub_cube[0], lp, cmax, zetp * dx[0], &idx3(handler->pol, 0, 0, 0)); /* k indice */
