...
void tensor_reduction_for_collocate_integrate(double* scratch,
                                              /* const int *pos, */
                                              const double alpha, const double beta, const bool* const orthogonal,
...
void
collocate_l0_blocked_xy(double* scratch, const double alpha, const int sizex, const int sizey, const int x0,
...
void
collocate_l0_blocked_xy_add(double* scratch, const double alpha, const int sizex, const int sizey, const int x0,
                            const int y0, const int ld, const struct tensor_* p_alpha_beta_reduced_)
{
    const double* __restrict py = &idx3(p_alpha_beta_reduced_[0], 1, 0, y0); /* j indice */
    const double* __restrict px = &idx3(p_alpha_beta_reduced_[0], 2, 0, x0); /* i indice */
...
void
collocate_l0_blocked_z(double* scratch, const double alpha, const double beta, const int z0,
                       const struct tensor_* p_alpha_beta_reduced_, struct tensor_* cube)
{
...
void
collocate_l0_blocked(double* scratch, const double alpha, const double beta, const int* const position,
                     const struct tensor_* p_alpha_beta_reduced_, struct tensor_* cube)
{
    assert(scratch != NULL);
...
*/
void
tensor_reduction_for_collocate_integrate_blocked(double* scratch, const double alpha, const int* const lower_block,
                                                 const int* const upper_block, const bool* const orthogonal,
                                                 const struct tensor_* const Exp, const struct tensor_* const co,
                                                 const struct tensor_* const p_alpha_beta_reduced_, struct tensor_* gr)
{
    assert(scratch != NULL);
    assert(p_alpha_beta_reduced_);
    assert(gr != NULL);
    ...
    initialize_tensor_3(&W, co->size[1] /* gamma */, gr->blockDim[1] /* j */, gr->blockDim[2] /* i */);

    initialize_tensor_3(&bk1, gr->block->size[0], gr->block->size[1], gr->block->size[2]);

    initialize_tensor_2(&exp_blocked, max(gr->block->size[0], gr->block->size[1]),
                        max(gr->block->size[1], gr->block->size[2]));

    initialize_tensor_2(&exp_blocked_2, max(gr->block->size[0], gr->block->size[1]),
                        max(gr->block->size[1], gr->block->size[2]));
    ...
    if (co->size[0] > 1) {
        ...
        m3 = (dgemm_params*)malloc(sizeof(dgemm_params) * (upper_block[0] - lower_block[0]));
        ...
        initialize_tensor_3(&T, co->size[0] /* alpha */, co->size[1] /* gamma */, gr->blockDim[1] /* j */);
        ...
        for (int y = 0; y < (upper_block[1] - lower_block[1]); y++) {
            ...
            for (int x = 0; x < (upper_block[2] - lower_block[2]); x++) {
                ...
                m2.b     = &idx3(p_alpha_beta_reduced_[0], 2, 0,
                             x * gr->blockDim[2]); // X_{alpha, i}  = p_alpha_beta_reduced(0, alpha, i)
                ...
                for (int z = 0; z < (upper_block[0] - lower_block[0]); z++) {
                    ...
                    m3[indz].a =
                        &idx3(p_alpha_beta_reduced_[0], 0, 0, z * gr->blockDim[0]); // p_alpha_beta_reduced(0, gamma, i)
...
#pragma GCC ivdep
    ...
    for (int y = 0; y < (upper_block[1] - lower_block[1]); y++) {
        ...
        for (int x = 0; x < (upper_block[2] - lower_block[2]); x++) {
            int x1 = (x + lower_block[2]) % gr->size[2];
            collocate_l0_blocked_xy(scratch, co->data[0] * alpha, gr->blockDim[2], gr->blockDim[1], x * gr->blockDim[2],
            ...
            for (int z = 0; z < (upper_block[0] - lower_block[0]); z++) {
                ...
                if (Exp && (!orthogonal[0] || !orthogonal[1])) {
                    ...
                    collocate_l0_blocked_z(scratch, co->data[0] * alpha, 0.0, z * gr->blockDim[0],
                                           p_alpha_beta_reduced_, gr->block);
...
#pragma GCC ivdep
                    ...
                    gr->block->data = &idx4(gr[0], z1, y1, x1, 0);
                    collocate_l0_blocked_z(scratch, co->data[0] * alpha, 1.0, z * gr->blockDim[0],
                                           p_alpha_beta_reduced_, gr->block);
                }
            }
        }
...
void
apply_mapping_cubic(const int* lower_boundaries_cube, const int* cube_center, const int* period, const tensor* cube,
        ...
        if (upper_corner[0] - lower_corner[0]) {
            ...
            for (int y = 0; y < cube->size[1]; y1++, y++) {
                ...
                if (upper_corner[1] - lower_corner[1]) {
                    ...
                    if ((lower_corner[0] > grid->size[0]) || (upper_corner[0] > grid->size[0]) ||
               ...
               const double radius)
{
    ...
    if (handler->use_gpu) {
        int position[3];
        return_cube_position(handler->grid.size, lb_grid, cubecenter, lb_cube, npts, position);
        ...
        add_orbital_to_list(handler->worker_list, handler->coef.size[2] - 1, cube_size, position, roffset, zetp,
                            &handler->coef);
    ...
    if (use_ortho) {
        grid_fill_pol(false, handler->dh[0][0], roffset[2], pol_offset[2], lb_cube[2], ub_cube[2],
                      handler->coef.size[2] - 1, cmax, zetp, &idx3(handler->pol, 2, 0, 0)); /* i indice */
        grid_fill_pol(false, handler->dh[1][1], roffset[1], pol_offset[1], lb_cube[1], ub_cube[1],
                      handler->coef.size[1] - 1, cmax, zetp, &idx3(handler->pol, 1, 0, 0)); /* j indice */
        grid_fill_pol(false, handler->dh[2][2], roffset[0], pol_offset[0], lb_cube[0], ub_cube[0],
                      handler->coef.size[0] - 1, cmax, zetp, &idx3(handler->pol, 0, 0, 0)); /* k indice */
    } else {
        grid_fill_pol(false, 1.0, roffset[0], pol_offset[2], lb_cube[0], ub_cube[0], handler->coef.size[0] - 1, cmax,
                      zetp * handler->dx[0], &idx3(handler->pol, 0, 0, 0)); /* k indice */
        grid_fill_pol(false, 1.0, roffset[1], pol_offset[1], lb_cube[1], ub_cube[1], handler->coef.size[1] - 1, cmax,
                      zetp * handler->dx[1], &idx3(handler->pol, 1, 0, 0)); /* j indice */
        ...
        if (blocked_decomposition) {
            ...
            calculate_non_orthorombic_corrections_tensor(zetp, roffset, handler->dh, lb_cube, ub_cube,
...
// *****************************************************************************
void
grid_collocate_pgf_product_cpu(void* const handle, const bool use_ortho, const int func, const int la_max,
                               const int la_min, const int lb_max, const int lb_min, const double zeta,
                               const double zetb, const double rscale, const double dh[3][3], const double dh_inv[3][3],
                               const double ra[3], const double rab[3], const int npts[3], const int ngrid[3],
                               const int lb_grid[3], const bool periodic[3], const double radius, const int o1,
                               const int o2, const int n1, const int n2, const double pab[n2][n1], double* grid_)
{
    ...
    initialize_grid(handler, use_ortho, false, dh, dh_inv, npts, lb_grid, ngrid, grid_);
    ...
    grid_prepare_alpha(ra, rb, rp, lmax_prep, &handler->alpha);
