...
__inline__ __device__ void  return_cube_position(const int3 *__restrict__ const cube_center,
...
__global__ void compute_collocation_gpu_spherical(const int3 grid_size_,
                                                  const int3 grid_lower_corner_pos_,
                                                  const int3 period_,
                                                  const int3 window_shift,
                                                  const int3 window_size,
                                                  const int *__restrict__ lmax_gpu_,
                                                  const double *__restrict__ zeta_gpu,
                                                  const double3 *__restrict__ rp,
                                                  const double *__restrict__ radius_gpu_,
                                                  ...
                                                  double *__restrict__ grid_gpu_)
{
    ...
    compute_cube_properties(radius,
                            rp + blockIdx.x,
                            &disr_radius,
                            &roffset,
                            &cube_center,
    ...
    return_cube_position(&cube_center,
                         &lb_cube,
                         period_,
                         &position);
...
extern "C"  void compute_collocation_gpu(pgf_list_gpu *handler)
{
    ...
    cudaMemcpyAsync(handler->rp_gpu_, handler->rp_cpu_, sizeof(double3) * handler->list_length, cudaMemcpyHostToDevice, handler->stream);
    cudaMemcpyAsync(handler->radius_gpu_, handler->radius_cpu_, sizeof(double) * handler->list_length, cudaMemcpyHostToDevice, handler->stream);
    ...
    compute_collocation_gpu_<<<block,
        ...
        (handler->lmax + 1) * (handler->lmax + 1) * (handler->lmax + 1) * sizeof(double), handler->stream>>>(handler->grid_size,
                                                                                                             ...
                                                                                                             handler->grid_full_size,
                                                                                                             handler->window_shift,
                                                                                                             handler->window_size,
                                                                                                             handler->lmax_gpu_,
                                                                                                             handler->zeta_gpu_,
                                                                                                             handler->rp_gpu_,
...
extern "C" void initialize_grid_parameters_on_gpu(collocation_integration *handler)
{
    ...
    dh_inv[0] = handler->dh_inv[0][0];
    dh_inv[1] = handler->dh_inv[0][1];
    dh_inv[2] = handler->dh_inv[0][2];
    dh_inv[3] = handler->dh_inv[1][0];
    dh_inv[4] = handler->dh_inv[1][1];
    dh_inv[5] = handler->dh_inv[1][2];
    dh_inv[6] = handler->dh_inv[2][0];
    dh_inv[7] = handler->dh_inv[2][1];
    dh_inv[8] = handler->dh_inv[2][2];

    int orthorhombic = 0;
    orthorhombic = handler->orthogonal[0] && handler->orthogonal[1] && handler->orthogonal[2];
    ...
    for (int worker = 0; worker < handler->worker_list_size; worker++) {
        ...
        handler->worker_list[worker].grid_lower_corner_position.x = handler->grid.lower_corner[2];
        handler->worker_list[worker].grid_lower_corner_position.y = handler->grid.lower_corner[1];
        handler->worker_list[worker].grid_lower_corner_position.z = handler->grid.lower_corner[0];

        cudaMalloc(&handler->worker_list[worker].data_gpu_, sizeof(double) * handler->grid.alloc_size_);
        cudaMemset(handler->worker_list[worker].data_gpu_, 0, sizeof(double) * handler->grid.alloc_size_);
