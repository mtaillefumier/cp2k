...
void *grid_gpu_create_task_list() {
		void *ctx =  malloc(sizeof(grid_gpu_task_list));
		memset(ctx, 0, sizeof(grid_gpu_task_list));
		return ctx;
}


static void grid_gpu_release_ressources(grid_gpu_task_list *task_list)
{
...
void grid_gpu_update_task_list(
		...
		grid_gpu_task_list *task_list = (grid_gpu_task_list *)ptr;

		// Select GPU device.
		CHECK(cudaSetDevice(grid_library_get_config().device_id));

		if (task_list != NULL) {
				grid_gpu_release_ressources(task_list);
				memset(task_list, 0, sizeof(grid_gpu_task_list));
		}
