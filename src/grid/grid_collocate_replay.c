...
//******************************************************************************
void
grid_collocate_record(const bool orthorhombic, const int border_mask, const int func, const int la_max,
                      const int la_min, const int lb_max, const int lb_min, const double zeta, const double zetb,
                      const double rscale, const double dh[3][3], const double dh_inv[3][3], const double ra[3],
                      const double rab[3], const int npts_global[3], const int npts_local[3], const int shift_local[3],
                      const int border_width[3], const double radius, const int o1, const int o2, const int n1,
                      const int n2, const double pab[n2][n1], const double* grid)
{
...
//******************************************************************************
static void
read_next_line(char line[], int length, FILE* fp)
{
...
//******************************************************************************
static void
parse_next_line(const char key[], FILE* fp, const char format[], const int nargs, ...)
{
    char line[100];
...
//******************************************************************************
static int
parse_int(const char key[], FILE* fp)
{
    int value;
...
//******************************************************************************
static void
parse_int3(const char key[], FILE* fp, int vec[3])
{
...
//******************************************************************************
static double
parse_double(const char key[], FILE* fp)
{
    double value;
...
//******************************************************************************
static void
parse_double3(const char key[], FILE* fp, double vec[3])
{
...
//******************************************************************************
static void
parse_double3x3(const char key[], FILE* fp, double mat[3][3])
{
    char format[100];
    for (int i = 0; i < 3; i++) {
...
//******************************************************************************
static void
create_dummy_basis_set(const int size, const int lmin, const int lmax, const double zet, grid_basis_set** basis_set)
{
...
//******************************************************************************
static void
create_dummy_task_list(const int border_mask, const double rscale, const double ra[3], const double rab[3],
                       const double radius, const grid_basis_set* basis_set_a, const grid_basis_set* basis_set_b,
                       const int n1, const int n2, const int o1, const int o2, const int la_max, const int lb_max,
                       const double pab[n2][n1], const int cycles, const int cycles_per_block,
                       grid_task_list** task_list)
{
    ...
    const double atom_positions[2][3]   = {{ra[0], ra[1], ra[2]}, {rab[0] + ra[0], rab[1] + ra[1], rab[2] + ra[2]}};
    ...
    grid_create_task_list(ntasks, nlevels, natoms, nkinds, nblocks, buffer_size, block_offsets, atom_positions,
                          atom_kinds, basis_sets, level_list, iatom_list, jatom_list, iset_list, jset_list, ipgf_list,
                          jpgf_list, border_mask_list, block_num_list, radius_list, rab_list, &blocks_buffer,
                          task_list);
...
//******************************************************************************
double
grid_collocate_replay(const char* filename, const int cycles, const bool batch, const int cycles_per_block)
{
    ...
    if (batch) {
        ...
        create_dummy_task_list(border_mask, rscale, ra, rab, radius, basisa, basisb, n1, n2, o1, o2, la_max, lb_max,
        ...
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start_time);
        for (int i = 0; i < cycles; i++) {
            grid_collocate_pgf_product(orthorhombic, border_mask, func, la_max, la_min, lb_max, lb_min, zeta, zetb,
                                       rscale, dh, dh_inv, ra, rab, npts_global, npts_local, shift_local, border_width,
                                       radius, o1, o2, n1, n2, pab, grid_test);
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end_time);
    }
