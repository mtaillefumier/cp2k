...
// *****************************************************************************
static void
grid_prepare_pab_AB(struct pab_computation_struct_* const tp)
{
        ...
        for (int lxb = 0; lxb <= tp->lmax[1]; lxb++) {
            for (int lya = 0; lya <= tp->lmax[0] - lxa; lya++) {
                for (int lyb = 0; lyb <= tp->lmax[1] - lxb; lyb++) {
                    for (int lza = max(tp->lmin[0] - lxa - lya, 0); lza <= tp->lmax[0] - lxa - lya; lza++) {
                        for (int lzb = max(tp->lmin[1] - lxb - lyb, 0); lzb <= tp->lmax[1] - lxb - lyb; lzb++) {
                            const int ico = tp->offset[0] + coset(lxa, lya, lza);
                            const int jco = tp->offset[1] + coset(lxb, lyb, lzb);
                            idx2(tp->pab_prep[0], coset(lxb, lyb, lzb), coset(lxa, lya, lza)) =
                                idx2(tp->pab[0], jco, ico);
                        }
                    }
                }
            }
...
// *****************************************************************************
static void
grid_prepare_pab_DADB(struct pab_computation_struct_* const tp)
{
    ...
    // (ddx pgf_a ) (ddx pgf_b) = (lax pgf_{a-1x} - 2*zeta*pgf_{a+1x})*(lbx pgf_{b-1x} - 2*tp->zeta[1]*pgf_{b+1x})
...
// *****************************************************************************
static void
grid_prepare_pab_ADBmDAB(struct pab_computation_struct_* const tp)
{
...
grid_prepare_pab_ARDBmDARB(struct pab_computation_struct_* const tp)
{
    ...
    //                        pgf_a *(lbx pgf_{b-1x+1ir} - 2*tp->zeta[1]*pgf_{b+1x+1ir}) -
...
// *****************************************************************************
static void
grid_prepare_pab_DABpADB(struct pab_computation_struct_* const tp)
{
...
// *****************************************************************************
static void
grid_prepare_pab_Di(struct pab_computation_struct_* const tp)
{
    ...
    // dx pgf_a dx pgf_b =
    //        (lax pgf_{a-1x})*(lbx pgf_{b-1x}) - 2*tp->zeta[1]*lax*pgf_{a-1x}*pgf_{b+1x} -
...
// *****************************************************************************
static void
grid_prepare_pab_DiDj(struct pab_computation_struct_* const tp)
{
                ...
                for (int lyb = 0; lyb <= tp->lmax[1] - lxb; lyb++) {
                    for (int lza = max(tp->lmin[0] - lxa - lya, 0); lza <= tp->lmax[0] - lxa - lya; lza++) {
                        for (int lzb = max(tp->lmin[1] - lxb - lyb, 0); lzb <= tp->lmax[1] - lxb - lyb; lzb++) {
                            ...
                            if ((tp->dir1 == 'X' && tp->dir2 == 'Y') || (tp->dir1 == 'Y' && tp->dir2 == 'X')) {
                                // xy
                                ico_l  = coset(max(lxa - 1, 0), max(lya - 1, 0), lza);
                                func_a = lxa * lya * pab;
                                oneterm_dijdij('X', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                                ...
                                oneterm_dijdij('X', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                            } else if ((tp->dir1 == 'Y' && tp->dir2 == 'Z') || (tp->dir1 == 'Z' && tp->dir2 == 'Y')) {
                                // yz
                                ico_l  = coset(lxa, max(lya - 1, 0), max(lza - 1, 0));
                                func_a = lya * lza * pab;
                                oneterm_dijdij('Y', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                                ...
                                func_a = 4.0 * tp->zeta[0] * tp->zeta[0] * pab;
                                oneterm_dijdij(2, func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                            } else if ((tp->dir1 == 'Z' && tp->dir2 == 'X') || (tp->dir1 == 'X' && tp->dir2 == 'Z')) {
                                // zx
                                ico_l  = coset(max(lxa - 1, 0), lya, max(lza - 1, 0));
                                func_a = lza * lxa * pab;
                                oneterm_dijdij('Z', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
...
// *****************************************************************************
static void
grid_prepare_pab_Di2(struct pab_computation_struct_* const tp)
{
                        ...
                        for (int lzb = max(tp->lmin[1] - lxb - lyb, 0); lzb <= tp->lmax[1] - lxb - lyb; lzb++) {
                            ...
                            switch (tp->dir1) {
                                case 'X': {
                                    ...
                                    oneterm_diidii('X', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                                ...
                                case 'Y': {
                                    ...
                                    oneterm_diidii('Y', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
                                ...
                                case 'Z': {
                                    ...
                                    oneterm_diidii('Z', func_a, ico_l, lxb, lyb, lzb, tp->zeta[1], tp->pab_prep);
...
// *****************************************************************************
void
grid_prepare_get_ldiffs_dgemm(const int func, int* const lmin_diff, int* const lmax_diff)
{
    switch (func) {
        case GRID_FUNC_AB:
            lmax_diff[0] = 0;
