             ...
             const int* lb_grid, tensor* cube)
{
    ...
    for (int z = 0; (z < cube->size[0]); z++, z1++) {
        /* see utils.h */
        lower_corner[0] = z1;
        upper_corner[0] = compute_next_boundaries(&z1, z, period[0], cube->size[0]);
        ...
        if (upper_corner[0] - lower_corner[0]) {
            ...
            for (int y = 0; y < cube->size[1]; y1++, y++) {
                /* see utils.h */
                lower_corner[1] = y1;
                upper_corner[1] = compute_next_boundaries(&y1, y, period[1], cube->size[1]);
                ...
                if (upper_corner[1] - lower_corner[1]) {
                    ...
                    for (int x = 0; x < cube->size[2]; x++, x1++) {
                        /* see utils.h */
                        lower_corner[2] = x1;
                        upper_corner[2] = compute_next_boundaries(&x1, x, period[2], cube->size[2]);
                        ...
                        update_loop_index(lower_corner[2], upper_corner[2], period[2], &x_offset, &x, &x1);
                    }
                }
                update_loop_index(lower_corner[1], upper_corner[1], period[1], &y_offset, &y, &y1);
            }
        }
        update_loop_index(lower_corner[0], upper_corner[0], period[0], &z_offset, &z, &z1);
    }
}
...
// *****************************************************************************
void
grid_integrate_pgf_product_cpu_dgemm(const bool use_ortho, const int lp, const double zeta, const double zetb,
