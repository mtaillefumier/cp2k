...
//******************************************************************************
static void
sum_stats(const grid_library_stats increment, grid_library_stats* accumulator)
{
...
//******************************************************************************
void
grid_library_gather_stats(const grid_library_stats increment)
{
    if (!library_initialized) {
        printf("Error: Grid library is not initialized.\n");
        abort();
...
//******************************************************************************
void
grid_library_print_stats(void (*mpi_sum_func)(long*), void (*print_func)(char*))
{
    if (!library_initialized) {
        printf("Error: Grid library is not initialized.\n");
        abort();
    ...
    snprintf(buffer, sizeof(buffer), " %-58s %20li\n", "ref_collocate_ortho", totals.ref_collocate_ortho);
    print_func(buffer);

    mpi_sum_func(&totals.ref_collocate_general);
    snprintf(buffer, sizeof(buffer), " %-58s %20li\n", "ref_collocate_general", totals.ref_collocate_general);
    print_func(buffer);
