...
#include "grid_ref_internal_header.h"
 ...
 * \author Ole Schuett
 ******************************************************************************/
static void grid_ref_release_ressources(void *ptr) {

		grid_ref_task_list *task_list = (grid_ref_task_list *)ptr;
		if (task_list->block_offsets)
				free(task_list->block_offsets);
		if (task_list->atom_positions)
				free(task_list->atom_positions);
		if (task_list->atom_kinds)
				free(task_list->atom_kinds);
		if (task_list->basis_sets)
				free(task_list->basis_sets);
		if (task_list->tasks)
				free(task_list->tasks);
		if (task_list->tasks_per_level)
				free(task_list->tasks_per_level);
		memset(task_list, 0, sizeof(grid_ref_task_list));
}
...
void grid_ref_update_task_list(
		...
		grid_ref_release_ressources(task_list);
		memset(task_list, 0, sizeof(grid_ref_task_list));
		...
		for (int i = 0; i < ntasks; i++) {
				...
				task_list->tasks[i].rab[0] = rab_list[3 * i];
				task_list->tasks[i].rab[1] = rab_list[3 * i + 1];
				task_list->tasks[i].rab[2] = rab_list[3 * i + 2];
		}
...
void grid_ref_collocate_task_list(
		...
		const grid_ref_task_list *task_list = (grid_ref_task_list *)ptr;
	...
	for (int level = 0; level < task_list->nlevels; level++) {
		...
		collocate_one_grid_level(task_list,
														 ...
														 &npts_local[3 * level],
														 &shift_local[3 * level],
														 &border_width[3 * level],
														 (double(*)[3])&dh[9 * level],
														 (double(*)[3])&dh_inv[9 * level],
														 pab_blocks->host_buffer,
														 grid[level]);
...
void grid_ref_integrate_task_list(
	...
	for (int level = 0; level < task_list->nlevels; level++) {
		...
		integrate_one_grid_level(task_list,
														 first_task,
														 last_task,
														 orthorhombic,
														 compute_tau,
