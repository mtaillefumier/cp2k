                             ...
                             const double dh[nlevels][3][3], const double dh_inv[nlevels][3][3], double* grid[nlevels])
{
    ...
    for (int level = 0; level < task_list->nlevels; level++) {
        ...
        collocate_one_grid_level(task_list, first_task, last_task, orthorhombic, func, npts_global[level],
                                 npts_local[level], shift_local[level], border_width[level], dh[level], dh_inv[level],
                                 grid[level]);
