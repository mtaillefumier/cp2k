...
//******************************************************************************
static void
prepare_alpha(const double ra[3], const double rb[3], const double rp[3], const int la_max, const int lb_max,
...
//******************************************************************************
static void
prepare_coef(const int la_max, const int la_min, const int lb_max, const int lb_min, const int lp,
             const double prefactor, const double alpha[3][lb_max + 1][la_max + 1][lp + 1],
             const double pab[ncoset[lb_max]][ncoset[la_max]], double coef_xyz[lp + 1][lp + 1][lp + 1])
{
...
//******************************************************************************
static void
fill_map(const int lb_cube, const int ub_cube, const int cubecenter, const int npts_global, const int shift_local,
...
//******************************************************************************
static void
fill_pol(const double dr, const double roffset, const int lb_cube, const int lp, const int cmax, const double zetp,
...
//******************************************************************************
static void
collocate_core_simple(const int lp, const int cmax, const double coef_xyz[lp + 1][lp + 1][lp + 1],
                      const double pol[3][lp + 1][2 * cmax + 1], const int lb_cube[3], const int ub_cube[3],
                      const double dh[3][3], const double dh_inv[3][3], const double disr_radius,
                      const int cubecenter[3], const int npts_global[3], const int npts_local[3],
                      const int shift_local[3], double* grid)
{
            ...
            for (int lxp = 0; lxp <= lp - lzp - lyp; lxp++) {
                for (int k = 0; k < nz; k++) {
                    for (int j = 0; j < ny; j++) {
                        for (int i = 0; i < nx; i++) {
                            cube[k * ny * nx + j * nx + i] +=
                                coef_xyz[lzp][lyp][lxp] * pol[2][lzp][k] * pol[1][lyp][j] * pol[0][lxp][i];
                        }
                    }
                }
            }
    ...
    for (int kg = kgmin; kg <= 1 - kgmin; kg++) {
        ...
        for (int jg = jgmin; jg <= 1 - jgmin; jg++) {
            ...
            for (int ig = igmin; ig <= 1 - igmin; ig++) {
                ...
                const int grid_index = k * npts_local[1] * npts_local[0] + j * npts_local[0] + i;
                const int cube_index = (kg - lb_cube[2]) * ny * nx + (jg - lb_cube[1]) * nx + (ig - lb_cube[0]);
...
//******************************************************************************
static void
collocate_core(const int lp, const int cmax, const double coef_xyz[lp + 1][lp + 1][lp + 1],
               const double pol[3][lp + 1][2 * cmax + 1], const int map[3][2 * cmax + 1], const int lb_cube[3],
               const double dh[3][3], const double dh_inv[3][3], const double disr_radius, const int npts_local[3],
               double* grid)
{
...
//******************************************************************************
static void
collocate_ortho(const int lp, const double zetp, const double coef_xyz[lp + 1][lp + 1][lp + 1], const double dh[3][3],
                const double dh_inv[3][3], const double rp[3], const int npts_global[3], const int npts_local[3],
                const int shift_local[3], const double radius, double* grid)
{
    ...
    const double(*pol)[lp + 1][2 * cmax + 1] = (const double(*)[lp + 1][2 * cmax + 1]) pol_mutable;

    // Enable to run a much simpler, but also slower implementation.
    if (false) {
        collocate_core_simple(lp, cmax, coef_xyz, pol, lb_cube, ub_cube, dh, dh_inv, disr_radius, cubecenter,
                              npts_global, npts_local, shift_local, grid);
    } else {
        // a mapping so that the ig corresponds to the right grid point
        int map_mutable[3][2 * cmax + 1];
        for (int i = 0; i < 3; i++) {
            fill_map(lb_cube[i], ub_cube[i], cubecenter[i], npts_global[i], shift_local[i], cmax, map_mutable[i]);
        }
        const int(*map)[2 * cmax + 1] = (const int(*)[2 * cmax + 1]) map_mutable;

        collocate_core(lp, cmax, coef_xyz, pol, map, lb_cube, dh, dh_inv, disr_radius, npts_local, grid);
    }
}
...
//******************************************************************************
static void
collocate_general(const int border_mask, const int lp, const double zetp, const double coef_xyz[lp + 1][lp + 1][lp + 1],
                  const double dh[3][3], const double dh_inv[3][3], const double rp[3], const int npts_global[3],
                  const int npts_local[3], const int shift_local[3], const int border_width[3], const double radius,
                  double* grid)
{
                            ...
                            for (int klz = 0; klz <= lpz; klz++) {
                                for (int jlz = 0; jlz <= lpz - klz; jlz++) {
                                    for (int ilz = 0; ilz <= lpz - klz - jlz; ilz++) {
                                        ...
                                        coef_ijk[lijk - 1] +=
                                            coef_xyz[lz][ly][lx] * hmatgridp[ilx][0][0] * hmatgridp[jlx][1][0] *
                                            hmatgridp[klx][2][0] * hmatgridp[ily][0][1] * hmatgridp[jly][1][1] *
                                            hmatgridp[kly][2][1] * hmatgridp[ilz][0][2] * hmatgridp[jlz][1][2] *
    ...
    for (int k = index_min[2]; k <= index_max[2]; k++) {
        ...
        for (int j = index_min[1]; j <= index_max[1]; j++) {
            ...
            for (int i = 0; i < 3; i++) {
                const double v = (0 - gp[0]) * dh[0][i] + (j - gp[1]) * dh[1][i] + (k - gp[2]) * dh[2][i];
                a += dh[0][i] * dh[0][i];
                b += 2.0 * v * dh[0][i];
                c += v * v;
            }
            ...
            for (int i = ismin; i <= ismax; i++) {
                ...
                const int grid_index = k_index * npts_local[1] * npts_local[0] + j_index * npts_local[0] + i_index;
                grid[grid_index] += res;
            }
        }
    }
...
//******************************************************************************
void
grid_ref_collocate_pgf_product(const bool orthorhombic, const int border_mask, const int func, const int la_max,
                               ...
                               const int npts_local[3], const int shift_local[3], const int border_width[3],
                               ...
                               const double pab[n2][n1], double* grid)
{
    ...
    grid_ref_prepare_get_ldiffs(func, &la_min_diff, &la_max_diff, &lb_min_diff, &lb_max_diff);
    ...
    prepare_coef(la_max_prep, la_min_prep, lb_max_prep, lb_min_prep, lp, prefactor, alpha, pab_prep, coef_xyz_mutable);
    const double(*coef_xyz)[lp + 1][lp + 1] = (const double(*)[lp + 1][lp + 1]) coef_xyz_mutable;
    ...
    if (orthorhombic && border_mask == 0) {
        ...
        collocate_ortho(lp, zetp, coef_xyz, dh, dh_inv, rp, npts_global, npts_local, shift_local, radius, grid);
    } else {
        grid_library_gather_stats((grid_library_stats){.ref_collocate_general = 1});
        collocate_general(border_mask, lp, zetp, coef_xyz, dh, dh_inv, rp, npts_global, npts_local, shift_local,
                          border_width, radius, grid);
    }
}
