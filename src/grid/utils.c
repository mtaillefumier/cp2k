...
void
compute_block_dimensions(const int* const grid_size, int* const blockDim)
{
...
void
decompose_grid_to_blocked_grid(const tensor* gr, struct tensor_* block_grid)
{
...
void
recompose_grid_from_blocked_grid(const struct tensor_* block_grid, tensor* gr)
{
    tensor tmp;
    initialize_tensor_3(&tmp, block_grid->blockDim[0], block_grid->blockDim[1], block_grid->blockDim[2]);
    int lower_corner[3], upper_corner[3];
                ...
                for (int z = 0; z < sizez; z++) {
                    for (int y = 0; y < sizey; y++) {
                        double* __restrict__ dst =
                            &idx3(gr[0], lower_corner[0] + z, lower_corner[1] + y, lower_corner[2]);
 ...
 * to the grid gr */
void
add_blocked_tensor_to_tensor(const struct tensor_* block_grid, tensor* gr)
{
    tensor tmp;
    initialize_tensor_3(&tmp, block_grid->blockDim[0], block_grid->blockDim[1], block_grid->blockDim[2]);
    int lower_corner[3], upper_corner[3];
            ...
            for (int x = 0; x < block_grid->size[2]; x++) {
                ...
                for (int z1 = 0; z1 < sizez; z1++) {
                    for (int y1 = 0; y1 < sizey; y1++) {
                        double* __restrict__ dst =
                            &idx3(gr[0], lower_corner[0] + z1, lower_corner[1] + y1, lower_corner[2]);
...
void
add_transpose_blocked_tensor_to_tensor(const struct tensor_* block_grid, tensor* gr)
{
    tensor tmp;
    initialize_tensor_3(&tmp, block_grid->blockDim[0], block_grid->blockDim[1], block_grid->blockDim[2]);
    int lower_corner[3], upper_corner[3];
 ...
 * to the grid gr */
void
compare_blocked_tensor_to_tensor(const struct tensor_* block_grid, tensor* gr)
{
    tensor tmp;
    initialize_tensor_3(&tmp, block_grid->blockDim[0], block_grid->blockDim[1], block_grid->blockDim[2]);
    int lower_corner[3], upper_corner[3];
...
void
initialize_tensor_blocked(struct tensor_* a, const int dim, const int* const sizes, const int* const blockDim)
{
    assert(a != NULL);
...
add_sub_grid(const int* lower_corner, const int* upper_corner, const int* position, const tensor* subgrid, tensor* grid)
{
    ...
    for (int d = 0; d < 3; d++) {
        if ((lower_corner[d] >= grid->size[d]) || (lower_corner[d] < 0) || (lower_corner[d] >= upper_corner[d]) ||
            (upper_corner[d] > grid->size[d]) || (upper_corner[d] <= 0) ||
...
void
add_sub_grid_with_pcb(const int* period, const int* position_inside_subgrid, const int* lower_corner,
                      const int* upper_corner, const tensor* subgrid, tensor* grid)
{
...
void
compute_block_boundaries(const int* blockDim, const int* lb_grid, const int* grid_size, const int* blocked_grid_size,
                         const int* period, const int* cube_center, const int* cube_size,
                         const int* lower_boundaries_cube, int* lower_block_corner, int* upper_block_corner,
                         int* pol_offsets)
{
    int position[3];
    return_cube_position(grid_size, lb_grid, cube_center, lower_boundaries_cube, period, position);
    pol_offsets[0] = 0;
    pol_offsets[1] = 0;
    pol_offsets[2] = 0;
...
int
return_exponents(const int index)
{

    // the possible indices are encoded as z * 2 ^ 16 + y * 2 ^ 8 + x.
    static const int exponents[] = {
        0,      1,      256,    65536,  2,      257,    512,    65537,  65792,  131072, 3,      258,    513,    768,
        65538,  65793,  66048,  131073, 131328, 196608, 4,      259,    514,    769,    1024,   65539,  65794,  66049,
        66304,  131074, 131329, 131584, 196609, 196864, 262144, 5,      260,    515,    770,    1025,   1280,   65540,
        65795,  66050,  66305,  66560,  131075, 131330, 131585, 131840, 196610, 196865, 197120, 262145, 262400, 327680,
        6,      261,    516,    771,    1026,   1281,   1536,   65541,  65796,  66051,  66306,  66561,  66816,  131076,
        131331, 131586, 131841, 132096, 196611, 196866, 197121, 197376, 262146, 262401, 262656, 327681, 327936, 393216,
        7,      262,    517,    772,    1027,   1282,   1537,   1792,   65542,  65797,  66052,  66307,  66562,  66817,
        67072,  131077, 131332, 131587, 131842, 132097, 132352, 196612, 196867, 197122, 197377, 197632, 262147, 262402,
        262657, 262912, 327682, 327937, 328192, 393217, 393472, 458752, 8,      263,    518,    773,    1028,   1283,
        1538,   1793,   2048,   65543,  65798,  66053,  66308,  66563,  66818,  67073,  67328,  131078, 131333, 131588,
        131843, 132098, 132353, 132608, 196613, 196868, 197123, 197378, 197633, 197888, 262148, 262403, 262658, 262913,
        263168, 327683, 327938, 328193, 328448, 393218, 393473, 393728, 458753, 459008, 524288, 9,      264,    519,
        774,    1029,   1284,   1539,   1794,   2049,   2304,   65544,  65799,  66054,  66309,  66564,  66819,  67074,
        67329,  67584,  131079, 131334, 131589, 131844, 132099, 132354, 132609, 132864, 196614, 196869, 197124, 197379,
        197634, 197889, 198144, 262149, 262404, 262659, 262914, 263169, 263424, 327684, 327939, 328194, 328449, 328704,
        393219, 393474, 393729, 393984, 458754, 459009, 459264, 524289, 524544, 589824};
